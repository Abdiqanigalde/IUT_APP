...
def download_pdf(reg_id):
    ...
    sig_t = Table(sig_rows, colWidths=[190, 160, 30, 130],
                   rowHeights=[15, 30] * len(sig_labels))
    sig_style = [
        ('FONTSIZE',   (0, 0), (-1, -1), 9),
    ...
    seal_img = _load_logo_flowable('registrar_seal.png', 105, 75)
    if seal_img:
        seal_img.hAlign = 'CENTER'
        stamp_visual = seal_img
    else:
        stamp_visual = Drawing(105, 105)
        stamp_visual.add(Circle(52, 55, 45, strokeColor=colors.HexColor('#9ca3af'),
                                 strokeWidth=1, strokeDashArray=(3, 2), fillColor=None))
        stamp_visual.add(String(52, 58, "OFFICIAL", fontSize=8, fillColor=colors.HexColor('#9ca3af'),
                                 textAnchor='middle'))
        stamp_visual.add(String(52, 47, "SEAL", fontSize=8, fillColor=colors.HexColor('#9ca3af'),
                                 textAnchor='middle'))
    ...
    def _draw_bottom_footer(canvas_obj, doc_obj):
        ...
        canvas_obj.drawCentredString(
            page_center, 30,
            "Board Bazar, Gazipur-1704, Bangladesh   Tel : +880-2-996691250, "
            "+880-2-996691252   PABX : +880-2-996691254-59   Fax : +880-2-996691260"
        )
        canvas_obj.drawCentredString(
            page_center, 19,
            "e-mail : registrar@iut-dhaka.edu   website : www.iutoic-dhaka.edu"
        )
        canvas_obj.restoreState()

    doc.build(elements, onFirstPage=_draw_bottom_footer, onLaterPages=_draw_bottom_footer)
    buf.seek(0)
