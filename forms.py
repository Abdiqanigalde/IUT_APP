...
def normalize_email(value):
    """Lowercase + strip email input at the form layer, so the exact same
    address typed with different casing is always treated as one account —
    matches how virtually every real-world system treats email addresses,
    and prevents duplicate accounts / failed logins caused by casing."""
...
class ForgotPasswordForm(FlaskForm):
    email = StringField('Email Address', filters=[normalize_email], validators=[DataRequired(), Email()])
...
class OfficerProfileForm(FlaskForm):
    ...
    email = StringField('Office Email', filters=[normalize_email], validators=[Optional(), Email()])
    login_email = StringField('Login Email (for officer portal)', filters=[normalize_email], validators=[DataRequired(), Email()])
